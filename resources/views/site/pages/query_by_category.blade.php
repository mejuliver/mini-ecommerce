					...
					<h2 class="title text-center">{{ $query_title }}</h2>
						@if($breadcrumbs[1]['name']==='All')
							@foreach($items as $x)
								@if($x->items->count()!==0)
								<div class="col-sm-12">
									<h2 class="text-transform-capitalize margin-bottom10px font-size18px c-orange font-700">{{ $x->cat_name }}</h2>
								</div>
								@endif
									...
									<div class="single-products">
										<div class="productinfo text-center">
											<a href="{{ url('/product/'.$i->item->link_name) }}">
												...
												@if($i->item->discounted)
												<h2 class="item-price"><span>{{ $i->item->discounted }}</span>  <span style="text-decoration:line-through;font-weight:300;font-size:12px;">{{ $i->item->price }}</span>
												</h2>
												@else
												<h2 class="item-price">{{ $i->item->price }}</h2>
												@endif
